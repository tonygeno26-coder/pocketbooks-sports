...
test('uses shared pb-theme.css (not a separate host palette)', function() {
  assert(hostHtml.includes('href="pb-theme.css"'));
  assert(hostHtml.includes('--pb-bg:'));
  assert(hostHtml.includes('--pb-accent:'));
});
...
test('settlement recording gated off by default', function() {
  assert(hostHtml.includes('function _hostSettlementRecordingEnabled'));
  assert(hostHtml.includes('SETTLEMENT_RECORDING_ENABLED'));
  assert(hostHtml.includes('Settlement recording is disabled') || hostHtml.includes('Recording stays off'));
});

test('Host:Local badge hidden outside local dev', function() {
  assert(hostHtml.includes('function _isLocalHostEnv'));
  assert(/!isDb\s*&&\s*!_isLocalHostEnv\(\)/.test(hostHtml));
});

test('host bets compact tab row with spaced counts', function() {
  assert(hostHtml.includes('host-bets-tab-count'));
  assert(hostHtml.includes('#host-bets-tabs'));
  assert(!/host-bets-tabs[\s\S]{0,400}flex-direction:column/.test(hostHtml));
});
