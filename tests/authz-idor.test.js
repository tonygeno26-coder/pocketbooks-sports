...
console.log('\n── FE / BE source gates ──');
test('FE prefers JWT subject for dashboard playerId (deferred survivor FE)', function () {
  // Deferred on cursor/away-authz-survivor — skip until that FE lands.
  if (!playerHtml.includes('_sessionActorIdFromToken')) {
    console.log('  ⏭  deferred: JWT subject helper not on this branch');
    return;
  }
...
test('FE notification load never invents fake rows (deferred survivor FE)', function () {
  if (!playerHtml.includes('Server-backed only')) {
    console.log('  ⏭  deferred: server-backed notif gate not on this branch');
    // Still assert we do not invent demo generators on mainline player.html
