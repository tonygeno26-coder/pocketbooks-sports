...
## Verdict (reconciled 2026-09-09 — FE `cursor/pre-beta-authz-club`)

**BE HTTP money surfaces hard-require club** (dashboards, cash-out, join, settlement IDs, mirror). FE must continue to send club-scoped tokens. Residual owner-gated: prod `cancel_bet_tx` soft club + phantom $1000 — **DO NOT APPLY**. Settlement recording stays OFF.

See BE `docs/AUTHORIZATION_AUDIT.md` + `docs/CLUB_ISOLATION_AUDIT.md` for endpoint matrix status (**FIXED** vs **OWNER-GATED**).
